...
export class MiniMapComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  /**
   * Verifica si un evento es de tipo MouseEvent
   */
  private isMouseEvent(event: unknown): event is MouseEvent {
    return event !== null && typeof event === 'object' && 'clientX' in event && 'clientY' in event;
  }

  /**
   * Maneja eventos de clic en el minimapa
   */
  private handleCanvasClick(event: Event): void {
    ...
    const rect = this.miniMapCanvasEl.nativeElement.getBoundingClientRect();
  ...
  /**
   * Maneja eventos de movimiento en el minimapa
   */
   ...
   */
  private handleCanvasMouseUp(event: Event): void {
    if (!this.canvasContext || !this._elements.length) return;
    if (!this.isMouseEvent(event)) return;
