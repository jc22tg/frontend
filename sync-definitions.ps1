# ================================================================
# SCRIPT DE SINCRONIZACIÓN COMMON-DEFINITIONS
# ================================================================
# Copia automáticamente las definiciones del backend al frontend
...
if (-not (Test-Path $source)) {
    Write-Host "❌ ERROR: No se encuentra el directorio source: $source" -ForegroundColor Red
    exit 1
}
...
$robocopyArgs = @($source, $target, "/E", "/XD", "node_modules", "/NFL", "/NDL", "/NJH", "/NJS")
& robocopy @robocopyArgs | Out-Null

# Robocopy return codes: 0-7 are success, >7 are errors
if ($LASTEXITCODE -gt 7) {
    Write-Host "❌ ERROR en robocopy: $LASTEXITCODE" -ForegroundColor Red
    exit 1
}

if ($Verbose) { Write-Host "   Archivos copiados exitosamente" -ForegroundColor Gray }
...
Write-Host "✅ Sincronización completada: $($copiedFiles.Count) archivos" -ForegroundColor Green

# Ejecutar correcciones post-sync automáticamente
